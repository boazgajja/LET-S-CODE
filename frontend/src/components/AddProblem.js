...
import { Plus, ArrowLeft } from 'lucide-react'; // Import Plus and ArrowLeft icons
...
const AddProblem = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserAddedProblems = async () => {
      try {
        ...
        console.error('Error fetching user added problems:', err.response?.data || err.message);
        setError('Failed to load your added problems. Please try again later.');
      }
    };
  ...
  // Function to handle going back to problem list
  const handleGoBack = () => {
    setShowForm(false);
    setJsonMode(false);
    setError(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Go back to problem list after successful submission
        setTimeout(() => {
          setSuccess(false);
          setShowForm(false);
          setJsonMode(false);
  ...
  return (
    <div className="ap_container">
      {!showForm ? (
        // Show user problems list
        <div className="ap_problems-list-view">
          <h1>My Problems</h1>
          ...
          {userAddedProblems.length > 0 ? (
                  ...
                  return (
                    <div key={problem._id} className="ap_user-problem-card">
                      <div className="ap_user-problem-info">
                        ...
                        <div className="ap_problem-stats">
                          <span className="ap_added-date">
                            Added: {new Date(problem.createdAt).toLocaleDateString()}
                          </span>
                          {problem.tags && problem.tags.length > 0 && (
                            <div className="ap_problem-tags">
                              {problem.tags.map((tag, index) => (
                                <span key={index} className="ap_tag">{tag}</span>
                              ))}
                            </div>
                          )}
                        </div>
            ...
            <div className="ap_no-problems">
              <h3>No Problems Added Yet</h3>
              <p>You haven't added any problems to the platform yet. Click the + button to add your first problem!</p>
            </div>
          )}
          
          {/* Floating action button to show form */}
          <div 
        ...
        // Show form
        <div className="ap_form-view">
          <div className="ap_form-header">
            <button 
              className="ap_back-button"
