...
router.get('/problems/user-added', authenticateToken, async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Error fetching user-added problems', error: error.message });
  }
});
...
// //get problem by mangodb id 
// // Get problem by MongoDB _id (the truly unique id)
// router.get('/problem/:mongoId', async (req, res) => {
//   try {
//     const { mongoId } = req.params;
//     const problem = await Problem.findById(mongoId);
//     if (!problem) {
...
//   } catch (error) {
//     res.status(500).json({ success: false, message: 'Error fetching problem by _id', error: error.message });
//   }
// });
